									...
									std::vector<std::shared_ptr<const keyvalue::Entry>> entries) const
{
	...
	if(job->context.empty()) {
		...
		job->num_left = num_query_threads;
		job->found.resize(job->options.max_results);
		for(int i = 0; i < num_query_threads; ++i) {
			query_threads->add_task(std::bind(&SearchEngine::query_task_0, this, job, num_query_threads, i));
		}
	}
}
...
void SearchEngine::query_callback_1(std::shared_ptr<query_job_t> job) const
{
	...
	job->tmp_results.reserve(job->found.size());
...
void SearchEngine::check_word_queue_2()
{
	const auto now = vnx::get_wall_time_seconds();
	while(!word_queue_2.empty()
			&& page_info_async->vnx_get_num_pending() < max_num_pending)
	{
		const auto iter = word_queue_2.begin();
		if(now - iter->first >= 0)
		{
			auto job = iter->second;
			page_info_async->get_value_locked(Variant(job->url_key), lock_timeout * 1000,
					std::bind(&SearchEngine::word_process_callback_2, this, job, std::placeholders::_1));
			word_queue_2.erase(iter);
		} else {
			break;
		}
	}
...
void SearchEngine::word_process_callback_1(std::shared_ptr<word_process_job_t> job)
{
	...
	{
		word_queue_2.emplace(vnx::get_wall_time_seconds() + commit_delay, job);
	}
}
...
void SearchEngine::query_task_0(std::shared_ptr<query_job_t> job,
