...
public:
	...
	uint64_t get_page_entries(const std::vector<uint32_t>& page_ids, 
			const std::function<void(std::vector<::vnx::search::page_entry_t>)>& _callback = std::function<void(std::vector<::vnx::search::page_entry_t>)>(),
			const std::function<void(const std::exception&)>& _error_callback = std::function<void(const std::exception&)>());
...
private:
	...
	std::map<uint64_t, std::pair<std::function<void(std::vector<::vnx::search::page_entry_t>)>, std::function<void(const std::exception&)>>> vnx_queue_get_page_entries;
