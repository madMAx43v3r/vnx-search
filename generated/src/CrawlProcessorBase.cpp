...
const vnx::Hash64 CrawlProcessorBase::VNX_CODE_HASH(0x89b86c6564983938ull);
	...
	:	Module::Module(_vnx_name)
{
	...
	vnx::read_config(vnx_name + ".retry_interval", retry_interval);
...
void CrawlProcessorBase::accept(vnx::Visitor& _visitor) const {
	...
	_visitor.type_field(_type_code->fields[2], 2); vnx::accept(_visitor, jump_cost);
	_visitor.type_field(_type_code->fields[3], 3); vnx::accept(_visitor, reload_interval);
	_visitor.type_field(_type_code->fields[4], 4); vnx::accept(_visitor, retry_interval);
	_visitor.type_field(_type_code->fields[5], 5); vnx::accept(_visitor, max_per_domain);
	_visitor.type_field(_type_code->fields[6], 6); vnx::accept(_visitor, max_per_minute);
	...
	_visitor.type_field(_type_code->fields[10], 10); vnx::accept(_visitor, root_urls);
	_visitor.type_field(_type_code->fields[11], 11); vnx::accept(_visitor, url_index_server);
	_visitor.type_field(_type_code->fields[12], 12); vnx::accept(_visitor, crawl_frontend_server);
...
void CrawlProcessorBase::write(std::ostream& _out) const {
	...
	_out << ", \"retry_interval\": "; vnx::write(_out, retry_interval);
...
void CrawlProcessorBase::read(std::istream& _in) {
	...
	for(const auto& _entry : _object) {
		...
		} else if(_entry.first == "max_per_domain") {
		...
		} else if(_entry.first == "retry_interval") {
			vnx::from_string(_entry.second, retry_interval);
...
vnx::Object CrawlProcessorBase::to_object() const {
	...
	_object["retry_interval"] = retry_interval;
...
void CrawlProcessorBase::from_object(const vnx::Object& _object) {
	for(const auto& _entry : _object.field) {
		if(_entry.first == "crawl_frontend_server") {
			...
			_entry.second.to(retry_interval);
		} else if(_entry.first == "root_urls") {
...
std::shared_ptr<vnx::TypeCode> CrawlProcessorBase::static_create_type_code() {
	...
	type_code->code_hash = vnx::Hash64(0x89b86c6564983938ull);
...
void read(TypeInput& in, ::vnx::search::CrawlProcessorBase& value, const TypeCode* type_code, const uint16_t* code) {
	...
	if(type_code->is_matched) {
		...
		}
		{
			...
			if(_field) {
				vnx::read_value(_buf + _field->offset, value.retry_interval, _field->code.data());
			}
		}
		{
	...
	for(const vnx::TypeField* _field : type_code->ext_fields) {
		switch(_field->native_index) {
			...
			case 10: vnx::read(in, value.root_urls, type_code, _field->code.data()); break;
			case 11: vnx::read(in, value.url_index_server, type_code, _field->code.data()); break;
			case 12: vnx::read(in, value.crawl_frontend_server, type_code, _field->code.data()); break;
...
void write(TypeOutput& out, const ::vnx::search::CrawlProcessorBase& value, const TypeCode* type_code, const uint16_t* code) {
	...
	vnx::write_value(_buf + 8, value.retry_interval);
	vnx::write_value(_buf + 12, value.max_per_domain);
	...
	vnx::write_value(_buf + 28, value.max_queue_ms);
	...
	vnx::write(out, value.root_urls, type_code, type_code->fields[10].code.data());
	vnx::write(out, value.url_index_server, type_code, type_code->fields[11].code.data());
	vnx::write(out, value.crawl_frontend_server, type_code, type_code->fields[12].code.data());
}
